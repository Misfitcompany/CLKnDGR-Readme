...
**Stop-loss (downside exit):** the counterpart to the gain exit, so a bag that keeps falling isn't held forever. If a held position drops to **its average cost minus a governable depth — 45% by default** (UPDATE_STOP_LOSS_TRIGGER; `0` disables it), the contract cuts a governable slice of it — **60% by default** (UPDATE_STOP_LOSS_SELL) — **on Qswap** at the monthly check. It scales out, never dumping the whole bag at once: a dying asset bleeds down over a few cuts while a recovering one keeps the rest. Both knobs move in 15-point steps. The recovered QU is split a hardcoded **90% back to trading capital / 10% burned to the execution-fee reserve** — keeping the engine funded exactly when losses have starved its normal profit-funded top-up. Because it's a realized loss, **nothing** goes to shareholders, Qearn, or the CCF. (A safety guard skips the sale if the bag is so worthless the proceeds wouldn't even cover the 100K-QU Qswap fee.)
