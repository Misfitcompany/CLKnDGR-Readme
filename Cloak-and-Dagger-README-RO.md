...
**Presetul 3 — Recuperare / mod de avarie (limp mode)** este special: contractul **continuă să tranzacționeze normal**, dar direcționează **70% din profit** către rezerva pentru taxe de execuție, plătind în continuare deponenților cei 30% reținuți (NAV-ul lor continuă să crească), suspendând doar plățile către acționari, Qearn, dev fund și CCF — astfel își reconstruiește rapid bugetul de taxe on-chain, din propriile câștiguri. Se aplică **automat** ori de câte ori rezerva pentru taxe de execuție scade sub pragul guvernabil (vezi UPDATE_EXEC_RESERVE_FLOOR), iar acționarii îl pot selecta și manual. Contractul revine la presetul ales de la sine **odată ce rezerva urcă înapoi la 10% peste prag** — un tampon de histerezis, ca să nu poată intra și ieși rapid din recuperare la limită. (Presetul ales anterior este păstrat; recuperarea nu îl suprascrie niciodată.)
...
### Parametri guvernabili (33 de tipuri de propuneri)
...
| 16 | **UPDATE_EXEC_RESERVE_FLOOR** | 50M QU | Stabilește pragul de siguranță al rezervei pentru taxe de execuție. Dacă bugetul de taxe on-chain scade sub el, contractul continuă să tranzacționeze, dar direcționează 70% din profit către rezerva de taxe (presetul de recuperare) până se reumple la 10% peste prag. Opțiuni: 0 (oprit, implicit), 1B, 5B, 10B sau 20B QU. |
...
| 18 | **UPDATE_VIX_FACTOR** | 50M QU | Reglează sensibilitatea Dagger la breakout-ul de volatilitate — cât de mult trebuie să crească volatilitatea recentă pe orizontul rapid (implicit 2 zile, guvernabil prin Type 30) a unui token peste propria bază pe orizontul lent (implicit 1 săptămână, guvernabil prin Type 31) înainte ca Dagger să înceapă să îl vâneze. Stocat ×100. Opțiuni (multiplicator al propriei baze): 0,09; 0,18; 0,37; 0,75; 1,5; 2 (implicit); 2,25; 2,75; 3,5; 4,5; 5 — trimise ca 9 / 18 / 37 / 75 / 150 / 200 / 225 / 275 / 350 / 450 / 500. Sub 1× Dagger vânează aproape mereu (pragul devine filtrul); mai mare = mai selectiv. |
...
| 20 | **UPDATE_VIX_PULSE_RATE** | 50M QU | De câte ori pe zi eșantionează Dagger prețul fiecărui pool pentru a-și actualiza citirea de volatilitate. Opțiuni: **1 (implicit)**, 2 sau 3 pe zi. Mai multe = detecție mai precisă/rapidă, dar cost de taxe mai mare; mai puține = mai ieftin, dar mai lent la observarea unui token care se încălzește. **Orizonturile** de volatilitate (rapid/lent) sunt levere guvernabile separate (Type 30/31) și sunt păstrate în *zile*, deci rămân fixe când se schimbă rata pulsului — doar frecvența de eșantionare se schimbă aici. |
...
| 29 | **DELETE_POOL** | 50M QU | **Eliberează permanent slotul unui pool** și îl scoate din scanarea de la fiecare tick. Este ultimul pas dintr-un flux deliberat de retragere cu mai multe voturi: **dezactivare (Type 2) → retragere/vânzare completă (Type 11 / 17) → ștergere**. Nu mută niciun fond în sine. **Protecție (guard):** pool-ul trebuie să fie deja dezactivat ȘI complet golit — fără tokenuri în rezervă, fără poziție Cloak deschisă și nimic în așteptare în recuperare. Dacă oricare este diferit de zero, propunerea este respinsă la depunere. La finalul epocii slotul este recuperat prin swap-and-shrink (ultimul pool intră în slotul eliberat, `poolCount` scade cu 1). Ștergerea este **ireversibilă și reindexează pool-urile**, dar activul poate fi **re-adăugat ulterior** prin ADD_POOL ca un pool nou. Folosește `poolIndex`. |
| 30 | **UPDATE_VIX_FAST_DAYS** | 50M QU | Setează **orizontul rapid EWMA al VIX** în zile — fereastra scurtă a cărei volatilitate medie este comparată cu baza lentă (Type 31) pentru a detecta un breakout. Mai mic = mai reactiv. Orizontul este un **timp** în zile și este independent de rata pulsului (Type 20): divizorul EWMA este `zile × pulsuri/zi`. Opțiuni: **2 (implicit)**, 3, 4, 5. |
| 31 | **UPDATE_VIX_SLOW_WEEKS** | 50M QU | Setează **baza lentă EWMA a VIX** în săptămâni — fereastra lungă față de care este măsurat orizontul rapid (Type 30). Trimis în **săptămâni**, stocat intern ca `săptămâni × 7` zile. Mai mare = o bază mai calmă, mai lentă. Opțiuni: **1 (implicit = 1 săptămână / 7 zile)**, 2, 3, 4. |
| 32 | **UPDATE_SWING_CADENCE** | 50M QU | Setează **cadența de verificare per-pool a Cloak** în zile — cât de des este reevaluat fiecare pool pentru o acțiune de cumpărare/vânzare Cloak (cooldown-ul comun per-pool între acțiuni). Implicit ~lunar. Mai mic = Cloak revizitează fiecare pool mai des (mai receptiv, dar mai multă activitate cu taxe de execuție). Opțiuni: **30 (implicit ≈ lunar)**, 20, 10, 5 zile. |
| 33 | **UPDATE_TICKRATE_PIN** | 50M QU | **Centură de siguranță de urgență** pentru rata de tick auto-vindecătoare (vezi *Rata de tick auto-vindecătoare* mai jos). `newValue` = **0** (auto/self-heal, implicit) sau **1–40** = fixează rata contractului la acel număr de tick-uri/secundă și îngheață auto-calibrarea. Necesară doar dacă auto-reglatorul se comportă anormal. |
...
## Rata de tick auto-vindecătoare

Temporizatoarele contractului — cadența lunară a Cloak, cooldown-urile Dagger, intervalul de eșantionare VIX — sunt toate numărate în **tick-uri**, iar rețeaua rulează astăzi la aproximativ **4 tick-uri pe secundă**. Dacă viteza de tick a Qubic s-ar schimba vreodată, fiecare dintre aceste temporizatoare ar devia în tăcere: „lunar” ar putea deveni discret la fiecare trei săptămâni, sau la fiecare șase.

În loc să fixeze rigid 4/sec și să spere că nu se schimbă niciodată (o schimbare ar necesita altfel re-implementarea întregului contract), CLKnDGR **măsoară singur viteza reală de tick**. O dată pe zi citește propriul ceas de consens al rețelei, calculează câte tick-uri au trecut efectiv în acea zi reală și integrează rezultatul într-o estimare netezită (`dayTicks`). Fiecare temporizator este apoi exprimat ca un multiplu al acelei estimări live, așa că fiecare își păstrează sensul din lumea reală indiferent cum se mișcă viteza de tick — fără re-implementare.

Fiindcă guvernează temporizarea unor bani reali, este protejată de bariere de siguranță:

- **Limitare la interval rezonabil** — sunt acceptate doar vitezele măsurate între **1 și 40 tick-uri/sec**; orice în afară este tratat ca o eroare și ignorat.
- **Netezire puternică** — se adaptează în aproximativ o săptămână, așa că fluctuațiile normale ale rețelei nu pot zgudui temporizatoarele; o schimbare reală, susținută, este totuși urmărită.
- **Pornire la rece** — până când are măsurătoarea unei zile întregi folosește cei 4/sec de azi, deci comportamentul la lansare este identic cu al unui contract cu rată fixă.
- **Fixare de urgență (centura de siguranță a guvernanței)** — dacă auto-reglatorul se comportă vreodată anormal, acționarii pot îngheța rata de tick la o valoare fixă prin **UPDATE_TICKRATE_PIN (Type 33)**, sau o pot readuce la `0` pentru a preda controlul înapoi modului automat.

Este determinist: ceasul de consens se citește identic pe fiecare computer care rulează contractul, deci toate calculează exact același `dayTicks`.
